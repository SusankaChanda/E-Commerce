...
class AllProductsSection extends Component {
  ...
  getProducts = async () => {
    ...
    const apiUrl = `https://apis.ccbp.in/products?sort_by=${activeOptionId}&category=${activeCategoryId}&title_search=${searchInput}&rating=${activeRatingId}&page=${currentPage}&limit=${itemsPerPage}`
    const options = {
      headers: {
  ...
  renderProductsListView = () => {
    const {productsList, activeOptionId, currentPage, itemsPerPage} = this.state
  ...
  changeRating = activeRatingId => {
    this.setState({activeRatingId, currentPage: 1}, this.getProducts)
  }

  changeCategory = activeCategoryId => {
    this.setState({activeCategoryId, currentPage: 1}, this.getProducts)
  }
